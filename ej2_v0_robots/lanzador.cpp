...
/*void LanzarProcesoDePlataforma(){
      int pid;
      if( (pid = fork()) <0 ){
            MENSAJE_ERROR("Error al hacer fork de generador de proceso plataforma");
            exit(1);
      }else if( pid == 0 ){//Hijo
            execlp( string("./" + Robots::Constantes::NOMBRE_PROCESO_PLATAFORMA).c_str(),
                  Robots::Constantes::NOMBRE_PROCESO_PLATAFORMA.c_str(), reinterpret_cast<char*>(0) );
...
int main( int argc, char** argv ){
    ...
    LanzarProcesoDePlataforma();*/
    LanzarProcesosDeRobotsDeArmadoYDespacho( config.ObtenerCantidadRobots() );
